...
input <- arrange(input, division_eng_name, date)
...
for (k in 1 : length(var_tocheck)){
  ...
  create_change_before <- sapply(0:16, function(x){
    paste0(i, "_b", x, "= lag(", i , ",", x, 
           ") - lag(", i, ",", x + 1, ")")
  })  %>%
    paste(collapse = ",")
  create_change_before <- paste("group_by(input, division_eng_name) %>% mutate(", create_change_before, ")")
  input <- eval(parse(text = create_change_before))
  firstvalue <- paste0("firstvalue <- input %>% group_by(division_eng_name) %>% 
                       filter(date == min(date)) %>% 
                       select(date,", i, ", division_eng_name) %>% 
                       mutate(firstvalue =", i, ")")
  firstvalue <- eval(parse(text = firstvalue))
  input <- merge(input, firstvalue[ , c("division_eng_name", "firstvalue")], by = "division_eng_name")
  create_b16 <- paste0("input %>% group_by(division_eng_name) %>% arrange(date) %>% mutate(", 
  ...
  for (j in 1:5){
    ...
    create_change_after <- paste("group_by(input, division_eng_name) %>% mutate(", create_change_after, ")")
...
write.csv(out, 'jp/pretrend_jp.csv')
