...
input <- arrange(input, areaName, date)
...
for (k in 1 : length(var_tocheck)){
  ...
  create_change_before <- sapply(0:15, function(x){
    ...
    paste(collapse = ",")
  create_change_before <- paste("group_by(input, areaName) %>% mutate(", create_change_before, ")")
  input <- eval(parse(text = create_change_before))
  firstvalue <- paste0("firstvalue <- input %>% group_by(areaName) %>% 
                       filter(date == min(date)) %>% 
                       select(date,", i, ", areaName) %>% 
                       mutate(firstvalue =", i, ")")
  firstvalue <- eval(parse(text = firstvalue))
  input <- merge(input, firstvalue[ , c("areaName", "firstvalue")], by = "areaName")
  create_b16 <- paste0("input %>% group_by(areaName) %>% arrange(date) %>% mutate(", 
  ...
  for (j in 1:5){
    ...
    create_change_after <- paste("group_by(input, areaName) %>% mutate(", create_change_after, ")")
...
out$Intervention <- c('School', 'Childcare', 'Retail', 'Restaurant',
                      'Sports indoor')
write.csv(out, 'uk/pretrend_uk.csv')
