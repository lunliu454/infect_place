...
pref <- select(input, division_eng_name, city_pop, city.density) %>% distinct
...
age <- age[, c(1 : 2, 22 : 29)]
age <- merge(name, age, by.x = 'Prefecture.1', by.y = 'pref')
pref <- merge(pref, age, by.x = 'division_eng_name', by.y = 'Prefecture')
...
pref <- merge(pref, gdp, by.x = 'division_eng_name', by.y = 'pref')
...
income <- income %>% 
  ...
  distinct
pref <- merge(pref, income, by.x = 'division_eng_name', by.y = 'place')
...
pref <- merge(pref, pref_mu, by.x = 'division_eng_name', by.y = 'pref')
...
#cor gdppc income 0.55
pref <- mutate(pref, y65 = y65to74 + y75)
t <- select(pref, mupop_dense, largest_density, y65, income)
t <- select(pref, mean_mupop_dense, mean_density, y65, income)
cor(t)
