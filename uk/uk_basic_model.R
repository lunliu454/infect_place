...
t <- input %>%
  ...
  select(colnames(input)[c(17 : 32, 50)])
...
# output for plot ####
est <- data.frame(intervention = c('School', 'Childcare', 'Retail+Large group in', 
                                   'Restaurant+Small group in+Culture', 
                                   #'Bar', 'Entertainment+Sports outdoor',
                                   'Sports indoor'), 
                  effect = m$beta[c(1:4, 7)],
                  se_effect = m$cse[c(1:4, 7)])
write.csv(est, 'plot/base_uk.csv', row.names = F)
# output table ####
...
for (i in 1 : length(m$beta)){
  ...
  p <- m$cpval[i]
  if (!is.na(p) & p <= 0.1 & p > 0.05){
    b <- paste0(b, '*')
  }
  if (!is.na(p) & p <= 0.05 & p > 0.01){
    b <- paste0(b, '**')
  }
  if (!is.na(p) & p <= 0.01){
    b <- paste0(b, '***')
...
out$Intervention <- c('School', 'Childcare', 'Retail+Large group in',
                      'Restaurant+Culture+Small group in',
                      'Bar', 'Entertainment+Sports outdoor',
                      'Sports indoor', 'Stay-at-home')
out[9 : 11, 'Intervention'] <- c('Observations', 'R-squared', 'Adjusted R-squared')
out[9 : 11, 'Estimate'] <- c(m$N, 
                              sprintf("%.3f", round(summary(m)$r.squared, 3)), 
                              sprintf("%.3f", round(summary(m)$adj.r.squared, 3)))
write.csv(out, 'table/output_base_uk.csv')

# joint effect ----
var <- c("win7_shop_close","win7_restaurant_close",
         "win7_sports_indoor_close") #school childcare not pass common trend
n <- length(var)
l <- rep(list(0:1), n)
l <- expand.grid(l)
...
write.csv(combined_result, "uk/uk_jointeffect_full.csv")
num_max <- combined_result %>% 
  group_by(number) %>% 
  top_n(-1, coefficient) %>%
  relocate(combine, number)
write.csv(num_max, "plot/combined_num_uk.csv")
